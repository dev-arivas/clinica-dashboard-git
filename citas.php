...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_cita'])) {
    // Aseguramos que solo usamos enteros, fechas y strings para seguridad
    ...
    if ($stmt->execute()) {
        ...
        // Manejo de error básico (ej. si paciente_id no existe)
        $error_msg = "Error al agendar cita: " . $stmt->error;
    }
    
    $stmt->close();
    header("Location: citas.php"); // Redirige para evitar el reenvío del formulario
    exit;
}

// Lógica de LEER CITAS (READ - usando JOIN para obtener el nombre del paciente)
...
// Obtener lista de pacientes para el formulario de selección (SELECT)
